...
const getTokenServiceEnable = prrocess.env.VITE_GET_TOKENS_SERVICE === 'true';
  ...
  observer(({ theme, ...props }: IStyledChildrenProps<BoxProps>) => {
    ...
    const goToBridge = () => {
      if (exchange.operation && exchange.operation.id) {
        routing.push(
          `/${exchange.token || TOKEN.ONE}/operations/${exchange.operation.id}`,
        );
      } else {
        routing.push(`/${exchange.token || TOKEN.ONE}`);
      }
    };
